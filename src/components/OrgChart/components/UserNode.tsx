...
const UserNode: React.FC<IUserNode> = ({ node }) => {
  const departmentStyle = clsx({
    ...
    'bg-pink-100 text-pink-500':
      node?.department?.name.toLowerCase() === 'marketing',
  ...
  if (node.parentId !== '') {
    return (
      <div
        id={node.id}
        className="flex flex-col rounded-9xl pt-3 px-2 pb-2 bg-white w-full h-full relative"
      >
        <div className="flex">
          <Avatar name={node.userName} image={node.profileImage} />
          <div className="flex flex-col ml-4">
            <div className="text-sm font-bold">
              {node.userName || 'Isabel Marcado'}
            </div>
            <div className="text-sm my-1">Sales Director</div>
            <div className="flex items-center">
              <div className="mr-1">
                <Icon name="location" size={24} stroke="black" />
              </div>
